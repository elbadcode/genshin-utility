...
ini_file& library::global_config()
{
    return ini_file::load_cache(ui::options::get_working_path() / L"library.ini");
}
...
bool ini_file::save()
{
	...
	return false; // File exists and was modified on disk and therefore may have different config, so cannot save
    ...
    const std::string str = config.str();
